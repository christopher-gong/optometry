...
def normHists(df, col):
    '''This fn returns three histograms for ease of comparison.'''
    return df[[col, "norm " + col, "z norm " + col]].hist(bins=1000,figsize=(20,10))

def meanSTD(df, cols, index):
    '''This fn will return a mean and std dataframe for each column in columnsAnalyze, and limit it to first index elements of the dataFrame.'''
    slideinfo = []
    unwantedCols2=[]
    selectCols = cols
